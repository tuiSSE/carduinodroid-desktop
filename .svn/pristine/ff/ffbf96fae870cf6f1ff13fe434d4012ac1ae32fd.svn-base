...
public class Direction_KeyListener implements KeyListener{
	...
	boolean up = false, left = false, right = false, down = false, front=true;
	...
	public void keyPressed(KeyEvent e) {	
		...
		if (key == KeyEvent.VK_UP) {up=true;controller_computer.car_controller.UpdateVariables(up,down,right,left);};
		...
		if (key == KeyEvent.VK_SHIFT){front=false;gui_computer.controller_Computer.car_controller.UpdateCamera(front);}
	}
