...
public class Socket_Package implements Runnable{
	 ...
	 */
	public boolean connect(InetSocketAddress nport_package)
	{
		port_package = nport_package;
		try {
			socket_package = new Socket();
			socket_package.connect(port_package,5000);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			/*
			 * noch überlegen
			 */
			System.out.println("Socket_Package - Fehler beim Connecten");
		}
		...
		try {
			...
			System.out.println("Socket_Package - Fehler beim Inputstream");
		}
		if(socket_package.isConnected()){
			System.out.println("Socket_Package - Connected");
			return true;
		}else return false;
	}
					...
					network.receive_package(message);
						...
						else
						{
							...
							try {
								...
								System.out.println("Socket_Package - Fehler beim Schlafen");
							}
						}
					}
				} catch (IOException e) {
					System.out.println("Socket_Package - Fehler beim Lesen");
				}
			}
		}
